...
async function createSF9() {
  ...
  function setupSheet(sheet, semData) {
    ...
    // Column E is increased slightly to help widen the left side.
        sheet.columns = [
      { width: 11.0 }, // A - Attendance Label (Wrap 2-3 lines)
      { width: 1.8 },  // B - Jun
      { width: 1.8 },  // C - Jul
      { width: 1.8 },  // D - Aug
      { width: 1.8 },  // E - Sept
    ...
    const drawBorder = (rangeStr, type = 'all', style = 'hair') => {
    ...
    sheet.getCell('G1').value = 'Republic of the Philippines';
    sheet.getCell('G1').font = { name: 'Arial', size: 10, bold: true };
    sheet.getCell('G1').alignment = { horizontal: 'center', vertical: 'middle' };

    sheet.mergeCells('G2:T2');
    sheet.getCell('G2').value = 'Department of Education';
    sheet.getCell('G2').font = { name: 'Arial', size: 11, bold: true };
    sheet.getCell('G2').alignment = { horizontal: 'center', vertical: 'middle' };

    sheet.mergeCells('G3:T3');
    sheet.getCell('G3').value = 'NCR\nManila\nII';
    sheet.getCell('G3').font = { name: 'Arial', size: 9, bold: true };
    sheet.getCell('G3').alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
    sheet.getRow(3).height = 42;

    sheet.mergeCells('G4:T4');
    sheet.getCell('G4').value = 'ACLC NORTHBAY';
    sheet.getCell('G4').font = { name: 'Arial', size: 14, bold: true };
    sheet.getCell('G4').alignment = { horizontal: 'center', vertical: 'middle' };

    sheet.mergeCells('G5:T5');
    sheet.getCell('G5').value = '2-3 Flr. MTSC Bldg. Juan Luna St. Cor. Capulong Tondo, Manila';
    sheet.getCell('G5').font = { name: 'Arial', size: 8 };
    ...
    sheet.getCell(`B${r}`).value = semData.nameLast;
    sheet.getCell(`B${r}`).font = { bold: true, size: 10 };
    sheet.getCell(`B${r}`).alignment = { horizontal: 'left', vertical: 'middle' };
    for (let c = 2; c <= 5; c++) sheet.getCell(r, c).border = { bottom: { style: 'thin' } };
    
    sheet.mergeCells(`F${r}:L${r}`);
    sheet.getCell(`F${r}`).value = semData.nameFirst;
    sheet.getCell(`F${r}`).font = { bold: true, size: 10 };
    sheet.getCell(`F${r}`).alignment = { horizontal: 'center', vertical: 'middle' };
    for (let c = 6; c <= 12; c++) sheet.getCell(r, c).border = { bottom: { style: 'thin' } };
    
    sheet.mergeCells(`M${r}:Y${r}`);
    sheet.getCell(`M${r}`).value = semData.nameMiddle;
    sheet.getCell(`M${r}`).font = { bold: true, size: 10 };
    sheet.getCell(`M${r}`).alignment = { horizontal: 'center', vertical: 'middle' };
    for (let c = 13; c <= 25; c++) sheet.getCell(r, c).border = { bottom: { style: 'thin' } };

    r = 9;
    sheet.getRow(r).height = 20;
    
    sheet.getCell(`A${r}`).value = 'USN:';
    sheet.getCell(`A${r}`).font = { bold: true, size: 10 };
    sheet.getCell(`A${r}`).alignment = { horizontal: 'left', vertical: 'middle' };

    sheet.mergeCells(`B${r}:F${r}`);
    sheet.getCell(`B${r}`).value = semData.usn;
    sheet.getCell(`B${r}`).font = { bold: true, size: 10 };
    sheet.getCell(`B${r}`).alignment = { horizontal: 'center', vertical: 'middle' };
    for (let c = 2; c <= 6; c++) sheet.getCell(r, c).border = { bottom: { style: 'thin' } };

    sheet.mergeCells(`G${r}:H${r}`);
    sheet.getCell(`G${r}`).value = 'LRN:';
    ...
    sheet.getCell(`I${r}`).value = semData.lrn;
    sheet.getCell(`I${r}`).font = { bold: true, size: 10 };
    sheet.getCell(`I${r}`).alignment = { horizontal: 'center', vertical: 'middle' };
    for (let c = 9; c <= 13; c++) sheet.getCell(r, c).border = { bottom: { style: 'thin' } };

    sheet.mergeCells(`N${r}:O${r}`);
    sheet.getCell(`N${r}`).value = 'AGE:';
    sheet.getCell(`N${r}`).font = { bold: true, size: 10 };
    sheet.getCell(`N${r}`).alignment = { horizontal: 'right', vertical: 'middle' };

    sheet.mergeCells(`P${r}:Q${r}`);
    sheet.getCell(`P${r}`).value = semData.age;
    sheet.getCell(`P${r}`).font = { bold: true, size: 10 };
    sheet.getCell(`P${r}`).alignment = { horizontal: 'center', vertical: 'middle' };
    sheet.getCell(`P${r}`).border = { bottom: { style: 'thin' } };
    sheet.getCell(`Q${r}`).border = { bottom: { style: 'thin' } };

    sheet.mergeCells(`R${r}:T${r}`);
    sheet.getCell(`R${r}`).value = 'Sex:';
    sheet.getCell(`R${r}`).font = { bold: true, size: 10 };
    sheet.getCell(`R${r}`).alignment = { horizontal: 'right', vertical: 'middle' };

    sheet.mergeCells(`U${r}:Y${r}`);
    sheet.getCell(`U${r}`).value = semData.sex;
    sheet.getCell(`U${r}`).font = { bold: true, size: 10 };
    sheet.getCell(`U${r}`).alignment = { horizontal: 'center', vertical: 'middle' };
    for (let c = 21; c <= 25; c++) sheet.getCell(r, c).border = { bottom: { style: 'thin' } };

    r = 10;
    sheet.getRow(r).height = 20;
    
    sheet.mergeCells(`A${r}:B${r}`);
    sheet.getCell(`A${r}`).value = 'School Year:';
    sheet.getCell(`A${r}`).font = { bold: true, size: 10 };
    sheet.getCell(`A${r}`).alignment = { horizontal: 'left', vertical: 'middle' };

    sheet.mergeCells(`C${r}:H${r}`);
    sheet.getCell(`C${r}`).value = semData.schoolYear;
    sheet.getCell(`C${r}`).font = { bold: true, size: 10 };
    sheet.getCell(`C${r}`).alignment = { horizontal: 'center', vertical: 'middle' };
    for (let c = 3; c <= 8; c++) sheet.getCell(r, c).border = { bottom: { style: 'thin' } };
    ...
    sheet.mergeCells(`L${r}:Y${r}`);
    sheet.getCell(`L${r}`).value = semData.track;
    sheet.getCell(`L${r}`).font = { bold: true, size: 10 };
    sheet.getCell(`L${r}`).alignment = { horizontal: 'center', vertical: 'middle' };
    for (let c = 12; c <= 25; c++) sheet.getCell(r, c).border = { bottom: { style: 'thin' } };
    ...
    sheet.getCell(`G${r}`).value = 'Subjects';
    sheet.getCell(`G${r}`).font = { bold: true, size: 10 };
    sheet.getCell(`G${r}`).alignment = { horizontal: 'center', vertical: 'middle' };

    sheet.mergeCells(`W${r}:X${r}`);
    sheet.getCell(`W${r}`).value = 'Quarter';
    sheet.getCell(`W${r}`).font = { bold: true, size: 10 };
    sheet.getCell(`W${r}`).alignment = { horizontal: 'center', vertical: 'middle' };

    sheet.getCell(`W${r+1}`).value = semData.q1;
    sheet.getCell(`W${r+1}`).font = { bold: true, size: 10 };
    sheet.getCell(`W${r+1}`).alignment = { horizontal: 'center', vertical: 'middle' };

    sheet.getCell(`X${r+1}`).value = semData.q2;
    ...
    sheet.getCell(`Y${r}`).value = "Semester\nFinal\nGrade"; // Exact formatting with \n mapping requested
    sheet.getCell(`Y${r}`).font = { bold: true, size: 9 };
    sheet.getCell(`Y${r}`).alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
    ...
    const addSubjectRow = (cat, code, title, q1, q2, fg, rem) => {
        if (cat) {
            ...
            drawBorder(`A${r}:Z${r}`, 'all'); // Applied medium border styling to dividers
            r++;
        }
        ...
        sheet.getCell(`G${r}`).value = title;
        sheet.getCell(`G${r}`).font = { size: 10 };
        sheet.getCell(`G${r}`).alignment = { vertical: 'middle', horizontal: 'left' };
        
        sheet.getCell(`W${r}`).value = q1;
        sheet.getCell(`W${r}`).alignment = { horizontal: 'center', vertical: 'middle' };
        sheet.getCell(`W${r}`).font = { size: 10 };
        
        sheet.getCell(`X${r}`).value = q2;
        sheet.getCell(`X${r}`).alignment = { horizontal: 'center', vertical: 'middle' };
        sheet.getCell(`X${r}`).font = { size: 10 };
        
        sheet.getCell(`Y${r}`).value = fg;
    ...
    sheet.getCell(`Y${r}`).value = semData.genAvg;
    sheet.getCell(`Y${r}`).font = { bold: true, size: 10 };
    sheet.getCell(`Y${r}`).alignment = { horizontal: 'center', vertical: 'middle' };
    ...
    sheet.getCell(`Q${splitR}`).value = "REPORT ON LEARNER'S OBSERVED VALUES";
    sheet.getCell(`Q${splitR}`).font = { bold: true, size: 10 };
    sheet.getCell(`Q${splitR}`).alignment = { vertical: 'middle' };
    ...
    semData.attendance.forEach((attRow, idx) => {
        ...
        sheet.getRow(currR).height = 20; // Reduced a little from 38 to close visual gap spacing
    ...
    sheet.getCell(`Q${splitR}`).value = 'Core Values';
    sheet.getCell(`Q${splitR}`).font = { bold: true, size: 9 };
    sheet.getCell(`Q${splitR}`).alignment = { horizontal: 'center', vertical: 'middle' };

    sheet.mergeCells(`S${splitR}:X${splitR+1}`);
    sheet.getCell(`S${splitR}`).value = 'Behavioral Statements';
    sheet.getCell(`S${splitR}`).font = { bold: true, size: 9 };
    sheet.getCell(`S${splitR}`).alignment = { horizontal: 'center', vertical: 'middle' };

    sheet.mergeCells(`Y${splitR}:Z${splitR}`);
    sheet.getCell(`Y${splitR}`).value = 'Quarter';
    sheet.getCell(`Y${splitR}`).font = { bold: true, size: 9 };
    sheet.getCell(`Y${splitR}`).alignment = { horizontal: 'center', vertical: 'middle' };

    sheet.getCell(`Y${splitR+1}`).value = semData.q1;
    sheet.getCell(`Y${splitR+1}`).font = { bold: true, size: 9 };
    sheet.getCell(`Y${splitR+1}`).alignment = { horizontal: 'center', vertical: 'middle' };
    ...
    const addValueRow = (core, behavior, v1, v2, mergeCoreLines) => {
        ...
        if (core) {
            ...
            sheet.getCell(`Q${valR}`).value = core;
            sheet.getCell(`Q${valR}`).font = { bold: true, size: 9 };
            sheet.getCell(`Q${valR}`).alignment = { wrapText: true, vertical: 'middle', horizontal: 'center' };
        }

        sheet.mergeCells(`S${valR}:X${valR}`);
        sheet.getCell(`S${valR}`).value = behavior;
        sheet.getCell(`S${valR}`).font = { size: 8 };
        sheet.getCell(`S${valR}`).alignment = { wrapText: true, vertical: 'middle', horizontal: 'left' };

        sheet.getCell(`Y${valR}`).value = v1;
        sheet.getCell(`Y${valR}`).font = { size: 9 };
        sheet.getCell(`Y${valR}`).alignment = { horizontal: 'center', vertical: 'middle' };
    ...
    sheet.getCell(`Q${sigR}`).value = 'Observed Values';
    sheet.getCell(`Q${sigR}`).font = { bold: true, size: 9 };
    sheet.getCell(`Q${sigR}`).alignment = { vertical: 'middle' };
    ...
    sheet.getCell(`D${sigR}`).value = 'Grading Scale';
    sheet.getCell(`D${sigR}`).font = { bold: true, size: 9 };
    sheet.getCell(`D${sigR}`).alignment = { horizontal: 'center', vertical: 'middle' };

    sheet.mergeCells(`I${sigR}:L${sigR}`);
    sheet.getCell(`I${sigR}`).value = 'Remarks';
    sheet.getCell(`I${sigR}`).font = { bold: true, size: 9 };
    sheet.getCell(`I${sigR}`).alignment = { horizontal: 'center', vertical: 'middle' };

    drawBorder(`A${sigR}:C${sigR}`, 'all');
    drawBorder(`D${sigR}:H${sigR}`, 'all');
    drawBorder(`I${sigR}:L${sigR}`, 'all');

    sheet.getCell(`Q${sigR}`).value = 'Marking';
    sheet.getCell(`Q${sigR}`).font = { bold: true, size: 9 };
    sheet.getCell(`Q${sigR}`).alignment = { horizontal: 'center', vertical: 'middle' };

    sheet.mergeCells(`R${sigR}:Z${sigR}`);
    sheet.getCell(`R${sigR}`).value = 'Non-numerical Rating';
    sheet.getCell(`R${sigR}`).font = { bold: true, size: 9 };
    sheet.getCell(`R${sigR}`).alignment = { horizontal: 'left', vertical: 'middle' };
    ...
    for(let i=0; i<5; i++) {
        ...
        sheet.getCell(`D${currR}`).value = d[1];
        sheet.getCell(`D${currR}`).font = { size: 9, color: { theme: 1 } };
        sheet.getCell(`D${currR}`).alignment = { vertical: 'middle', horizontal: 'center' };
        
        sheet.mergeCells(`I${currR}:L${currR}`);
        sheet.getCell(`I${currR}`).value = d[2];
        sheet.getCell(`I${currR}`).font = { size: 9, color: { theme: 1 } };
        sheet.getCell(`I${currR}`).alignment = { vertical: 'middle', horizontal: 'center' };
        ...
        if(v[0]) {
            sheet.getCell(`Q${currR}`).value = v[0];
            sheet.getCell(`Q${currR}`).font = { size: 9, color: { theme: 1 } };
            sheet.getCell(`Q${currR}`).alignment = { horizontal: 'center', vertical: 'middle' };

            sheet.mergeCells(`R${currR}:Z${currR}`);
            sheet.getCell(`R${currR}`).value = v[1];
            sheet.getCell(`R${currR}`).font = { size: 9, color: { theme: 1 } };
            sheet.getCell(`R${currR}`).alignment = { vertical: 'middle', horizontal: 'left' };
    ...
    sheet.mergeCells(`A${sigR}:H${sigR}`);
    sheet.getCell(`A${sigR}`).value = semData.adviser;
    sheet.getCell(`A${sigR}`).font = { bold: true, size: 10 };
    sheet.getCell(`A${sigR}`).alignment = { horizontal: 'center', vertical: 'bottom' };
    for(let c=1; c<=8; c++) sheet.getCell(sigR, c).border = { bottom: { style: 'thin' } };

    sheet.mergeCells(`I${sigR}:Q${sigR}`);
    sheet.getCell(`I${sigR}`).value = semData.principal;
    sheet.getCell(`I${sigR}`).font = { bold: true, size: 10 };
    sheet.getCell(`I${sigR}`).alignment = { horizontal: 'center', vertical: 'bottom' };
    for(let c=9; c<=17; c++) sheet.getCell(sigR, c).border = { bottom: { style: 'thin' } };

    sheet.mergeCells(`R${sigR}:Y${sigR}`);
    sheet.getCell(`R${sigR}`).value = semData.registrar;
    sheet.getCell(`R${sigR}`).font = { bold: true, size: 10 };
    sheet.getCell(`R${sigR}`).alignment = { horizontal: 'center', vertical: 'bottom' };
    for(let c=18; c<=25; c++) sheet.getCell(sigR, c).border = { bottom: { style: 'thin' } };

    sigR++;
    sheet.getRow(sigR).height = 15;

    sheet.mergeCells(`A${sigR}:H${sigR}`);
    sheet.getCell(`A${sigR}`).value = 'Adviser';
    sheet.getCell(`A${sigR}`).font = { size: 9 };
    sheet.getCell(`A${sigR}`).alignment = { horizontal: 'center', vertical: 'top' };

    sheet.mergeCells(`I${sigR}:Q${sigR}`);
    sheet.getCell(`I${sigR}`).value = 'SHS Head/Principal';
    sheet.getCell(`I${sigR}`).font = { size: 9 };
    sheet.getCell(`I${sigR}`).alignment = { horizontal: 'center', vertical: 'top' };

    sheet.mergeCells(`R${sigR}:Y${sigR}`);
    sheet.getCell(`R${sigR}`).value = 'Registrar';
    sheet.getCell(`R${sigR}`).font = { size: 9 };
    sheet.getCell(`R${sigR}`).alignment = { horizontal: 'center', vertical: 'top' };

    sigR += 3;
    sheet.getRow(sigR).height = 25;
    sheet.mergeCells(`A${sigR}:Y${sigR}`);
    sheet.getCell(`A${sigR}`).value = 'Note:\nThis document is generated automatically and is only valid when it bears the School/Registrar\'s Office seal. Any erasure or alteration made in this copy renders the whole report card INVALID. Print Date: ' + semData.printDate;
    sheet.getCell(`A${sigR}`).font = { size: 8, italic: true };
    sheet.getCell(`A${sigR}`).alignment = { wrapText: true, vertical: 'top', horizontal: 'left' };
  }
  ...
  const targetFile = path.join(process.cwd(), 'F-138_Template_v17.xlsx');
