...
export const DocumentViewerDialog = memo(({ 
  open, onOpenChange, file, rotation, setRotation, 
            ...
            <div className="w-full h-full absolute inset-0 flex items-center justify-center overflow-hidden z-10">
              {isPdf ? (
                <div className="w-full h-full p-4 md:p-12">
                  {/* Safely fallback to an empty string if file is null */}
                  <iframe src={file?.url || ""} className={`w-full h-full rounded-2xl shadow-2xl border ${isDarkMode ? 'bg-[#1a1a1a] border-white/5' : 'bg-white border-slate-200'}`} title="PDF Viewer" />
                </div>
              ) : (
                <TransformWrapper
                  initialScale={1}
