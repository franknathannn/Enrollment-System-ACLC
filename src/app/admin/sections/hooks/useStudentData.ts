      ...
      const [studentsRes, configRes, sectionsRes] = await Promise.all([
        supabase.from('students')
          .select([
            ...
            'gender', 'strand', 'contact_no', 'phone', 'email',
            'gwa_grade_10', 'profile_picture', 'status', 'decline_reason',
            'guardian_name', 'guardian_contact', 'section_id', 'school_year',
            'form_138_url', 'good_moral_url', 'created_at',
            ...
            'oed_usn', 'voucher_status', 'is_payee', 'grade_level',
          ].join(', '))
