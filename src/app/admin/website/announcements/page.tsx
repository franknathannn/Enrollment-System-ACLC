...
export default function AnnouncementsAdminPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('public_refresh_landing')
    channel.subscribe()
  ...
  const selectAnnouncement = (announcement: any) => {
    ...
    setCategory(announcement.category || "news")
  ...
  const handleSave = async () => {
    ...
    try {
      if (selectedId && selectedId !== "new") {
        const { error } = await supabase.from('announcements_landing').update(payload).eq('id', selectedId)
        if (error) throw error
      } else {
        const { error, data } = await supabase.from('announcements_landing').insert([payload]).select()
        if (error) throw error
      ...
      // Broadcast refresh signal to public clients (bypasses RLS dropping drafted rows)
      supabase.channel('public_refresh_landing').send({
        type: 'broadcast',
        event: 'refresh',
        payload: { source: 'announcements' }
      })
  ...
  const handleDelete = async (id: string) => {
    ...
    const { error } = await supabase.from('announcements_landing').delete().eq('id', id)
    if (error) {
      toast.error(error.message, { id: toastId })
    } else {
    ...
    <div className={`h-[calc(100vh-8rem)] flex overflow-hidden rounded-2xl border shadow-sm ${isDarkMode ? 'bg-slate-950 border-slate-800' : 'bg-white border-slate-200'}`}>
      ...
      <div className={`w-1/3 flex flex-col border-r ${isDarkMode ? 'border-slate-800 bg-slate-900/30' : 'border-slate-200 bg-slate-50'}`}>
        <div className={`p-4 border-b flex items-center justify-between ${isDarkMode ? 'border-slate-800' : 'border-slate-200'}`}>
          <div>
            <h1 className="text-xl font-black uppercase tracking-tighter">News</h1>
            <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Manage Articles</p>
          </div>
      ...
      <div className={`flex-1 flex flex-col ${isDarkMode ? 'bg-slate-950' : 'bg-white'}`}>
        {!selectedId ? (
          <div className="flex-1 flex flex-col items-center justify-center text-slate-400 p-8 text-center">
            ...
            <h2 className="text-xl font-bold uppercase tracking-wider mb-2">Select an Article</h2>
            <p className="text-sm max-w-sm">Choose an article from the sidebar to start writing, or create a new one.</p>
          </div>
        ) : (
          <>
            {/* Editor Topbar */}
            ...
            <div className="flex-1 overflow-y-auto p-6 lg:p-10 fancy-scroll">
              <div className="max-w-4xl mx-auto space-y-8">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Article Title</label>
                  <Input
                    type="text"
                    value={title}
                    onChange={(e) => {
                    ...
                    placeholder="e.g. Campus Events and News..."
                ...
                <div className="flex flex-wrap gap-6 items-center">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select value={category} onValueChange={setCategory}>
                      <SelectTrigger className={`h-9 border-none rounded-lg shadow-inner text-sm w-[160px] ${isDarkMode ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-900'}`}>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent className={`shadow-xl ${isDarkMode ? 'bg-slate-900 border-slate-700 text-white' : 'bg-white border-slate-200 text-slate-900'}`}>
                ...
                <div className="space-y-2 h-[500px] flex flex-col">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Article Content</label>
                  <MarkdownEditor 
                    value={body} 
                    onChange={setBody} 
