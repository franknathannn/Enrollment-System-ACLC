...
const Attachment = ({ msg, isMe }: { msg: any; isMe: boolean }) => {
  ...
  return (
    <button type="button" onClick={() => forceDownload(url, name)}
      className={cn("flex items-center gap-3 mt-2 p-3 rounded-xl border shadow-sm transition-all active:scale-95 w-full max-w-xs text-left hover:scale-[1.02]",
        isMe ? "bg-white/10 border-white/20 hover:bg-white/20" : "bg-blue-50/50 dark:bg-slate-800/50 border-blue-100 dark:border-slate-700 hover:bg-blue-100 dark:hover:bg-slate-700"
      )}>
                ...
                {showEmojiPicker && (
                  <>
                    ...
                    <div className={cn("absolute z-50 flex items-center p-1.5 border shadow-2xl animate-in zoom-in-75 fade-in duration-200 rounded-full gap-0.5",
                      ...
                      style={{ boxShadow: isDarkMode ? "0 10px 40px rgba(0,0,0,0.5)" : "0 10px 40px rgba(0,0,0,0.15)" }}>
...
export function GlobalChatPanel({ currentUser }: GlobalChatPanelProps) {
  ...
  // Fetch admin's own profile + build avatar map
  useEffect(() => {
  ...
  }, [newMessage, currentUser, isUploading])
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const { error: dbError } = await supabase.from("teacher_global_chat_messages").insert({
        content: "", sender_id: currentUser.id, sender_type: "admin",
  ...
  const saveEdit = useCallback(async () => {
    ...
    const { error } = await supabase.from("teacher_global_chat_messages").update({ content: editContent.trim() }).eq("id", editingId)
    if (error) { toast.error("Failed to edit."); return }
    setEditingId(null); setEditContent("")
  }, [editingId, editContent])

  const deleteMsg = useCallback(async (id: any) => {
    const { error } = await supabase.from("teacher_global_chat_messages").delete().eq("id", id)
  ...
  const toggleReaction = useCallback(async (msgId: any, emoji: string) => {
    ...
    const { error } = await supabase.from("teacher_message_reactions").insert({
      message_type: "global", message_id: String(msgId), emoji,
    ...
    <div className="flex flex-col h-full">
      ...
      <div className={cn("px-5 py-3 border-b shrink-0 flex items-center gap-3", isDarkMode ? "border-white/5" : "border-slate-100")}>
        ...
        <p className={cn("text-[10px] font-black uppercase tracking-widest", isDarkMode ? "text-slate-400" : "text-slate-500")}>
          Global Channel · Admins & Teachers
        </p>
      </div>
