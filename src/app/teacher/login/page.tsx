...
import { Loader2, GraduationCap, ShieldCheck, Eye, EyeOff, ArrowLeft } from "lucide-react"
    ...
    <div className={cn("min-h-screen w-full flex flex-col items-center justify-center p-6 py-12 relative overflow-hidden transition-colors duration-500", isDarkMode ? "bg-slate-950" : "bg-slate-50")}>
      ...
      <div className="max-w-md w-full space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700 relative z-10">
          ...
          <div className={cn("p-8 rounded-[39px] space-y-6", panelClass)}>
            ...
            <form onSubmit={handleLogin} className="space-y-5">
              ...
              <div className="space-y-2 group">
                ...
                <div className="relative">
                  <Input
                    ...
                    className={cn("h-14 rounded-2xl font-bold px-5 focus:border-blue-500 transition-all pr-12", inputClass)}
                    required disabled={loading}
                  />
              ...
              <Button type="submit" disabled={loading || failedAttempts >= MAX_ATTEMPTS}
                ...
                {loading ? <Loader2 className="animate-spin" size={18} /> : <><ShieldCheck size={16} /> Sign In</>}
              </Button>
            </form>
          </div>
        </Card>
        ...
        <div className="mt-8 flex flex-col items-center gap-4 z-10 w-full relative">
          ...
          <p className={cn("text-[9px] uppercase tracking-[0.5em] font-black italic", isDarkMode ? "text-slate-500" : "text-slate-400")}>Faculty • ACLC Northbay</p>
        </div>
      </div>
    </div>
  )
