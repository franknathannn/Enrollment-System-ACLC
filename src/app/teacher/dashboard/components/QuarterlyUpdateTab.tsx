    ...
    <div className="animate-in fade-in duration-300 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-gradient-to-br from-blue-600 to-indigo-600 rounded-[32px] p-6 sm:p-8 text-white relative overflow-hidden shadow-lg shadow-blue-500/20 gap-4">
        ...
        <div className="relative z-10 w-full sm:w-auto">
          <h2 className="text-2xl sm:text-3xl font-black uppercase tracking-tight">Academic Logs</h2>
          <p className="text-[10px] sm:text-xs font-bold uppercase tracking-widest text-blue-200 mt-1">Review assignments and submit requirements</p>
        </div>
          ...
          {requirements.map(req => {
            ...
            if (subm) {
                 ...
                 <div className="flex flex-col md:items-end gap-2 text-right">
                    <div className="flex items-center gap-2">
                       <span className={`px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-widest ${
                          subm.status === 'Accepted' ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20' :
                          subm.status === 'Invalidated' ? 'bg-rose-500/10 text-rose-500 border border-rose-500/20' :
                          'bg-blue-500/10 text-blue-500 border border-blue-500/20'
                       }`}>{subm.status}</span>
                       
                       <span className={`px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-widest ${
                         isLate ? 'bg-rose-500/10 text-rose-500 border border-rose-500/20' : 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20'
               ...
               statusMarkup = (
                  <div className="flex items-center gap-2">
                     <span className={`px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-widest ${
                        isPastDl ? 'bg-rose-500/10 text-rose-500 border border-rose-500/20' : dm ? 'bg-slate-800 text-slate-400' : 'bg-slate-100 text-slate-500'
