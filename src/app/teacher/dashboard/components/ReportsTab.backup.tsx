      ...
      <div id="print-area" className="hidden print:block font-sans" style={{ fontFamily: "Georgia, 'Times New Roman', serif", padding: "32px 40px", color: "#111" }}>
        ...
        {printSubject ? (() => {
          ...
          const studentStats = sectionStudents.map(student => {
            ...
            const hasCuttingNote = recs.some(r => r.notes === "CUTTING")
            const isCutting = absent >= COUNSELING_THRESHOLD
            const isEndangered = pct <= 40 || hasCuttingNote
            return { student, present, late, absent, excused, total, pct, isCutting, isEndangered }
          }).sort((a, b) => b.absent - a.absent)
          const flaggedStudents = studentStats.filter(s => s.isCutting || s.isEndangered)
          ...
          return (
            <div>
              ...
              <div style={{ background: "#111827", color: "#fff", borderRadius: "8px", padding: "14px 20px", marginBottom: "20px" }}>
                <p style={{ fontSize: "9px", color: "#9ca3af", letterSpacing: "0.1em", marginBottom: "3px" }}>Subject</p>
                <p style={{ fontSize: "20px", fontWeight: 900, letterSpacing: "-0.02em", margin: 0 }}>{subject}</p>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "11px" }}>
                <thead>
                  <tr style={{ borderBottom: "2px solid #111827" }}>
                    {["#", "Student Name", "LRN", "P", "L", "E", "A", "Att%", "Status"].map(h => (
                ...
                <tbody>
                  {studentStats.map(({ student, present, late, absent, excused, pct, isCutting, isEndangered }, i) => {
                    const rowBg = isCutting ? "#fff1f2" : isEndangered ? "#fef2f2" : i % 2 === 0 ? "#fff" : "#f9fafb"
                    const statusText = isCutting ? "Cutting Risk" : isEndangered ? "Danger" : pct >= 80 ? "Regular" : pct >= 60 ? "Fair" : "At Risk"
                    const statusColor = isCutting || isEndangered ? "#dc2626" : pct >= 80 ? "#16a34a" : pct >= 60 ? "#2563eb" : "#d97706"
                    return (
                      <tr key={student.id} style={{ backgroundColor: rowBg, borderBottom: "1px solid #f3f4f6" }}>
                        ...
                        <td style={{ padding: "6px 8px", fontWeight: 600, color: isCutting || isEndangered ? "#dc2626" : "#111827" }}>
                          {isEndangered && <span style={{ display: "inline-block", width: "6px", height: "6px", borderRadius: "50%", backgroundColor: "#ef4444", marginRight: "5px", verticalAlign: "middle" }} />}
                        ...
                        <td style={{ padding: "6px 8px", textAlign: "center", color: "#d97706", fontWeight: 600 }}>{late}</td>
                        ...
                        <td style={{ padding: "6px 8px", textAlign: "center", fontWeight: 700, color: pct >= 80 ? "#16a34a" : pct >= 60 ? "#2563eb" : "#dc2626" }}>{pct}%</td>
                        <td style={{ padding: "6px 8px", textAlign: "center", fontSize: "9px", fontWeight: 700, color: statusColor }}>{statusText}</td>
                      </tr>
                    )
                  })}
                </tbody>
              ...
              {flaggedStudents.length > 0 && (
                <div style={{ marginTop: "24px", padding: "14px 16px", border: "1px solid #fca5a5", borderRadius: "6px", background: "#fff5f5" }}>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "11px" }}>
                    ...
                    <tbody>
                      {flaggedStudents.map(({ student, absent, pct, isCutting, isEndangered }) => (
                        <tr key={student.id} style={{ borderBottom: "1px solid #fee2e2" }}>
                          ...
                          <td style={{ padding: "5px 8px", fontSize: "9px", color: "#9ca3af" }}>
                            {isCutting ? `${absent} absences in ${subject} — counseling required` : `Attendance at ${pct}% — below safe threshold`}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <div className={`px-6 md:px-10 pb-10 space-y-8 animate-in slide-in-from-top-4 duration-500`}>
                  ...
                  {subjectFocus[report.section] && (() => {
                    ...
                    const allStudentStats = sectionStudents.map(student => {
                      ...
                      const isCutting = absent >= COUNSELING_THRESHOLD
                      const isEndangered = pct <= 40 || hasCuttingNote
                      return { student, present, late, absent, excused, total, pct, isCutting, hasCuttingNote, isEndangered }
                    }).sort((a, b) => b.absent - a.absent)
                    ...
                    const StatusBadge = ({ pct, isCutting, isEndangered }: { pct: number; isCutting: boolean; isEndangered: boolean }) => {
                      if (isCutting) return (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-rose-500/15 text-rose-500 text-[7px] font-black uppercase tracking-widest border border-rose-500/20 whitespace-nowrap">
                          <Zap size={7} className="fill-rose-500" /> Cutting
                        </span>
                      )
                      if (isEndangered) return (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-red-500/10 text-red-600 text-[7px] font-black uppercase tracking-widest border border-red-500/15 whitespace-nowrap">
                          <AlertCircle size={7} /> Danger
                        </span>
                      )
                      if (pct < 60) return (
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[7px] font-black uppercase tracking-widest ${dm ? "bg-amber-500/10 text-amber-400" : "bg-amber-50 text-amber-600"} border border-amber-500/10`}>
                          At Risk
                        </span>
                      )
                      if (pct < 80) return (
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[7px] font-black uppercase tracking-widest ${dm ? "bg-blue-500/10 text-blue-400" : "bg-blue-50 text-blue-600"} border border-blue-500/10`}>
                          Fair
                        </span>
                      )
                      return (
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[7px] font-black uppercase tracking-widest ${dm ? "bg-white/5 text-slate-400" : "bg-emerald-50 text-emerald-600"}`}>
                          Regular
                        </span>
                      )
                    }
                      ...
                      <div className="animate-in slide-in-from-top-4 duration-500">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-4 px-1">
                          <div className="flex items-center gap-2 flex-1 min-w-0">
                            ...
                            <p className={`text-[9px] font-black uppercase tracking-widest truncate ${sub}`}>{focusedSubject} — Roster</p>
                          ...
                          <table className="w-full">
                            <thead>
                              <tr className={`${dm ? "bg-white/5" : "bg-slate-50"}`}>
                                ...
                                <th className="px-3 py-3 text-center text-[7px] font-black uppercase tracking-widest text-amber-500">L</th>
                            ...
                            <tbody className={`divide-y ${divB}`}>
                              ...
                              ) : visibleStats.map(({ student, present, late, absent, excused, pct, isCutting, isEndangered }, i) => (
                                <tr key={student.id}
                                  className={`transition-colors ${isCutting
                                    ? dm ? "bg-rose-500/10 hover:bg-rose-500/15" : "bg-rose-50/60 hover:bg-rose-100/60"
                                    : isEndangered
                                      ? dm ? "bg-red-900/10 hover:bg-red-900/15" : "bg-red-50/40 hover:bg-red-100/40"
                                      : dm ? "hover:bg-white/5" : "hover:bg-slate-50/80"}`}
                                >
                                  <td className={`px-4 py-2.5 text-[8px] font-black opacity-40 ${sub}`}>{i + 1}</td>
                                  <td className="px-4 py-2.5">
                                    <div className="flex items-center gap-2.5">
                                      {/* Red dot indicator for endangered students */}
                                      {isEndangered && (
                                        <span className="w-2 h-2 rounded-full bg-red-500 shrink-0 shadow-[0_0_6px_rgba(239,68,68,0.6)] animate-pulse" />
                                      )}
                                      <div className={`w-6 h-6 rounded-lg overflow-hidden shrink-0 ${dm ? "bg-white/5" : "bg-slate-100"} ${isEndangered ? "" : ""}`}>
                                      ...
                                      <div className="min-w-0">
                                        <span className={`block text-[10px] font-black uppercase tracking-tight leading-none ${isCutting ? "text-rose-500" : isEndangered ? "text-red-500" : head}`}>
                                          {student.last_name}, {student.first_name}
                                        </span>
                                        {student.lrn && <span className={`text-[7px] font-bold opacity-40 ${sub}`}>{student.lrn}</span>}
                                      </div>
                                    </div>
                                  </td>
                                  ...
                                  <td className="px-3 py-2.5 text-center">
                                    <span className={`text-[10px] font-black ${pct >= 80 ? "text-emerald-500" : pct >= 60 ? "text-blue-400" : pct >= 40 ? "text-amber-500" : "text-rose-500"}`}>{pct}%</span>
                                  </td>
                                  <td className="px-4 py-2.5 text-center">
                                    <StatusBadge pct={pct} isCutting={isCutting} isEndangered={isEndangered} />
                                  </td>
                                </tr>
                              ))}
                            </tbody>
