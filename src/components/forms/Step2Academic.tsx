...
export default function Step2Academic() {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      if (existingLrn) {
        toast.error("LRN is already registered in the database.")
        setChecking(false)
        return
      }
      ...
      updateFormData(data)
      setStep(3)
      toast.success("Academic Background Submitted", {
  ...
  const getFieldClassName = (fieldName: string, isLRN = false) => {
    ...
    return cn(
      "h-11 md:h-12 rounded-xl border-2 transition-all duration-300 font-medium outline-none text-white",
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-right-6 duration-1000 pb-10">
      ...
      <div className="bg-blue-600/10 rounded-[32px] p-6 border border-blue-500/20 text-white flex items-center gap-5 shadow-2xl relative overflow-hidden group">
        ...
        <div className="relative z-10">
          <p className="text-[9px] font-bold uppercase tracking-[0.4em] text-blue-400 mb-1">Step 02</p>
          <h2 className="text-xl md:text-2xl font-bold tracking-tight uppercase italic text-white leading-none">Academic Background</h2>
        </div>
      </div>

      {/* ROW 1: LRN */}
      <div className="relative z-[100] space-y-2 group">
        <Label htmlFor="lrn" className={cn("font-bold text-[10px] uppercase tracking-[0.3em] ml-2 transition-colors group-focus-within:text-blue-400", errors.lrn ? 'text-red-500' : 'text-slate-500')}>
        ...
        <div className="relative">
          ...
          <Input 
            ...
            maxLength={12}
            disabled={!isFieldEditable("lrn")}
            className={cn("pl-12", getFieldClassName("lrn", true), !isFieldEditable("lrn") && "opacity-50 cursor-not-allowed")}
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 relative z-[90]">
        <div className="space-y-2 group">
          <Label className="text-slate-500 font-bold text-[10px] uppercase tracking-[0.3em] ml-2 transition-colors group-focus-within:text-blue-400">
            Intel Classification {isFieldRequired("student_category") && <span className="text-red-500">*</span>}
          </Label>
          <Select 
            onValueChange={(v) => {
          ...
          >
            <SelectTrigger className={cn(
                "h-11 md:h-12 rounded-xl border-2 font-medium text-sm transition-all text-white focus:ring-0",
                ...
                !isFieldEditable("student_category") && "opacity-50 cursor-not-allowed"
            )}>
              <SelectValue placeholder="Category" />
            </SelectTrigger>
            <SelectContent className="rounded-2xl shadow-2xl border border-white/10 bg-slate-900 text-white">
              <SelectItem value="JHS Graduate" className="font-medium py-3 focus:bg-blue-600 focus:text-white">JHS Graduate</SelectItem>
              <SelectItem value="ALS Passer" className="font-medium py-3 focus:bg-blue-600 focus:text-white">ALS Passer</SelectItem>
            </SelectContent>
          </Select>
        </div>

        <div className="space-y-2">
          <Label className="text-slate-500 font-bold text-[10px] uppercase tracking-[0.3em] ml-2">Active Temporal Cycle</Label>
          <div className="h-11 md:h-12 rounded-xl border border-white/10 bg-slate-950/30 flex items-center px-4 gap-3 text-white shadow-inner">
            {fetchingYear ? <Loader2 size={16} className="animate-spin text-blue-600" /> : <CalendarDays size={16} className="text-blue-700" />}
            <span className="font-bold uppercase tracking-tighter text-[10px] md:text-xs text-slate-400">
      ...
      <div className="relative z-[80] space-y-2 group">
        <Label className={cn("font-bold text-[10px] uppercase tracking-[0.3em] ml-2 transition-colors group-focus-within:text-blue-400", errors.strand ? 'text-red-500' : 'text-slate-500')}>
          Strand Preferrence {isFieldRequired("strand") && <span className="text-red-500">*</span>}
        </Label>
        <Select 
          onValueChange={(v) => {
        ...
        >
          <SelectTrigger className={cn(
              ...
              !isFieldEditable("strand") && "opacity-50 cursor-not-allowed"
          )}>
            <SelectValue placeholder="Choose specialized division" />
          </SelectTrigger>
          <SelectContent className="rounded-2xl shadow-2xl border border-white/10 bg-slate-900 text-white">
            <SelectItem value="ICT" className="font-medium py-3 focus:bg-blue-600 focus:text-white">Information and Communication Technology (ICT) </SelectItem>
            <SelectItem value="GAS" className="font-medium py-3 focus:bg-blue-600 focus:text-white">General Academics (GAS)</SelectItem>
          </SelectContent>
        </Select>
      </div>
      ...
      <div className="relative z-[10] space-y-2 group">
        <Label htmlFor="last_school" className={cn("font-bold text-[10px] uppercase tracking-[0.3em] ml-2 transition-colors group-focus-within:text-blue-400", errors.last_school_attended ? 'text-red-500' : 'text-slate-500')}>
          Previous School {isFieldRequired("last_school_attended") && <span className="text-red-500">*</span>}
        </Label>
        <div className="relative">
            ...
            <Input 
                {...register("last_school_attended", { required: isFieldRequired("last_school_attended") })} 
                id="last_school"
                placeholder="PREVIOUS SCHOOL NAME" 
                disabled={!isFieldEditable("last_school_attended")}
                className={cn("pl-11 md:pl-12", getFieldClassName("last_school_attended"), !isFieldEditable("last_school_attended") && "opacity-50 cursor-not-allowed")}
            />
        </div>
      </div>
      ...
      {selectedCategory === "JHS Graduate" && (
        <div className="relative z-[0] space-y-2 group animate-in slide-in-from-top-4">
          <div className="flex justify-between items-center">
            <Label htmlFor="gwa" className={cn("font-bold text-[10px] uppercase tracking-[0.3em] ml-2 transition-colors group-focus-within:text-blue-400", errors.gwa_grade_10 ? 'text-red-500' : 'text-slate-500')}>
              General Weighted Average (GWA) {isFieldRequired("gwa_grade_10") && <span className="text-red-500">*</span>}
            </Label>
            {errors.gwa_grade_10 && (
              <span className="text-[9px] font-bold text-red-500 uppercase tracking-widest flex items-center gap-1 animate-in fade-in slide-in-from-right-2">
          ...
          <div className="relative">
            ...
            <Input 
              {...register("gwa_grade_10", { 
                required: isFieldRequired("gwa_grade_10") && selectedCategory === "JHS Graduate",
              ...
              step="0.01" 
              placeholder="00.00" 
              disabled={!isFieldEditable("gwa_grade_10")}
              className={cn("pl-12", getFieldClassName("gwa_grade_10"), !isFieldEditable("gwa_grade_10") && "opacity-50 cursor-not-allowed")}
            />
          </div>
        </div>
      )}
      ...
      <div className="pt-10 flex flex-col gap-4">
        <Button 
          ...
          className="h-14 md:h-16 rounded-[28px] bg-blue-600 hover:bg-white hover:text-blue-600 text-white shadow-[0_20px_50px_rgba(59,130,246,0.3)] transition-all duration-500 active:scale-95 flex items-center justify-center gap-4 group"
        >
          {checking ? (
            <><Loader2 className="animate-spin" /> Verifying LRN...</>
          ) : (
            <>
              <span className="font-bold uppercase text-xs tracking-[0.4em] ml-4 text-white group-hover:text-blue-600">Analyze Step 03</span>
        ...
        <button 
          type="button" 
          onClick={() => setStep(1)} 
          className="text-slate-600 font-bold uppercase text-[9px] tracking-[0.4em] flex items-center justify-center gap-2 hover:text-white transition-colors py-4 group"
        >
