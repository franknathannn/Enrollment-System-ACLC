...
export default function Step3Family() {
  ...
  const getFieldClassName = (fieldName: string) => {
    ...
    return cn(
      "h-11 md:h-12 rounded-xl border-2 transition-all duration-300 font-medium outline-none text-white text-sm",
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 animate-in fade-in slide-in-from-right-6 duration-1000 pb-10">
      ...
      <div className="bg-blue-600/10 rounded-[32px] p-6 border border-blue-500/20 text-white flex items-center gap-5 shadow-2xl relative overflow-hidden group">
        ...
        <div className="relative z-10">
          <p className="text-[9px] font-bold uppercase tracking-[0.4em] text-blue-400 mb-1">Step 03</p>
          <h2 className="text-xl md:text-2xl font-bold tracking-tight uppercase italic text-white leading-none">Family & Contacts</h2>
        </div>
      </div>

      {/* GUARDIAN IDENTITY MATRIX */}
      <div className="space-y-4">
        <Label className="text-slate-500 font-bold text-[10px] uppercase tracking-[0.3em] ml-2">
          Guardian Information {(isFieldRequired("guardian_first_name") || isFieldRequired("guardian_last_name")) && <span className="text-red-500">*</span>}
        </Label>
                ...
                <Input 
                  {...register(field as any, { 
                    ...
                    onChange: (e) => {
                      ...
                      const val = e.target.value.replace(/[^a-zA-Z\s]/g, '').split(' ').map((w: string) => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(' ');
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
        <div className="space-y-2 group/field">
          <Label className="text-slate-500 font-bold text-[10px] uppercase tracking-[0.3em] flex items-center gap-2 ml-2 transition-colors group-focus-within/field:text-blue-400">
             <Phone size={12} className="text-blue-700 transition-colors group-focus-within/field:text-blue-400" /> 
             Guardian Contact No. {isFieldRequired("guardian_phone") && <span className="text-red-500">*</span>}
          </Label>
          <Input 
            {...register("guardian_phone", { 
              required: isFieldRequired("guardian_phone"),
            ...
            maxLength={11}
            disabled={!isFieldEditable("guardian_phone")}
            className={cn(getFieldClassName("guardian_phone"), !isFieldEditable("guardian_phone") && "opacity-50 cursor-not-allowed")}
          />
        </div>
        ...
        <div className="space-y-2 group/field">
          <Label className="text-slate-500 font-bold text-[10px] uppercase tracking-[0.3em] flex items-center gap-2 ml-2 transition-colors group-focus-within/field:text-blue-400">
             <Phone size={12} className="text-blue-700 transition-colors group-focus-within/field:text-blue-400" /> 
             Student No. {isFieldRequired("phone") && <span className="text-red-500">*</span>}
          </Label>
          <Input 
            {...register("phone", { 
            ...
            className={cn(getFieldClassName("phone"), !isFieldEditable("phone") && "opacity-50 cursor-not-allowed")}
          />
        </div>
      </div>
      ...
      <div className="pt-10 flex flex-col gap-4">
        ...
        >
          <span className="font-bold uppercase text-xs tracking-[0.4em] ml-4 text-white group-hover:text-blue-600">Analyze Step 04</span>
