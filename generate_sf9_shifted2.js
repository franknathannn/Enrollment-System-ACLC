...
async function createSF9() {
  ...
  function setupSheet(sheet, semData) {
    ...
    sheet.getCell(`D${r}`).value = '      ' + semData.nameLast; // Added spacing before name value starts
    sheet.getCell(`D${r}`).font = { bold: true, size: 10 };
    sheet.getCell(`D${r}`).alignment = { horizontal: 'left', vertical: 'middle' };
    sheet.getCell(`D${r}`).border = { bottom: { style: 'thin' } };
    sheet.getCell(`E${r}`).border = { bottom: { style: 'thin' } };
    sheet.getCell(`F${r}`).border = { bottom: { style: 'thin' } };
    
    sheet.mergeCells(`G${r}:M${r}`);
    sheet.getCell(`G${r}`).value = semData.nameFirst;
    ...
    sheet.getCell(`N${r}`).value = semData.nameMiddle;
    sheet.getCell(`N${r}`).font = { bold: true, size: 10 };
    sheet.getCell(`N${r}`).alignment = { horizontal: 'center', vertical: 'middle' };
    for (let c = 13; c <= 24; c++) {
      sheet.getCell(r, c).border = { bottom: { style: 'thin' } };
    }
    ...
    sheet.getCell(`J${r}`).value = semData.lrn;
    sheet.getCell(`J${r}`).font = { bold: true, size: 10 };
    sheet.getCell(`J${r}`).alignment = { horizontal: 'left', vertical: 'middle' };
    for (let c = 9; c <= 13; c++) {
      sheet.getCell(r, c).border = { bottom: { style: 'thin' } };
    }

    sheet.mergeCells(`P${r}:Q${r}`);
    sheet.getCell(`P${r}`).value = 'AGE:';
    sheet.getCell(`P${r}`).font = { bold: true, size: 10 };
    sheet.getCell(`P${r}`).alignment = { horizontal: 'right', vertical: 'middle' };

    sheet.getCell(`R${r}`).value = semData.age;
    sheet.getCell(`R${r}`).font = { bold: true, size: 10 };
    sheet.getCell(`R${r}`).alignment = { horizontal: 'center', vertical: 'middle' };
    sheet.getCell(`R${r}`).border = { bottom: { style: 'thin' } };

    sheet.mergeCells(`S${r}:U${r}`);
    sheet.getCell(`S${r}`).value = 'Sex:';
    sheet.getCell(`S${r}`).font = { bold: true, size: 10 };
    sheet.getCell(`S${r}`).alignment = { horizontal: 'right', vertical: 'middle' };

    sheet.getCell(`V${r}`).value = semData.sex;
    sheet.getCell(`V${r}`).font = { bold: true, size: 10 };
    sheet.getCell(`V${r}`).alignment = { horizontal: 'center', vertical: 'middle' };
    sheet.getCell(`V${r}`).border = { bottom: { style: 'thin' } };
    ...
    sheet.getCell(`H${r}`).value = 'Track/Strand:';
    sheet.getCell(`H${r}`).font = { bold: true, size: 10 };
    sheet.getCell(`H${r}`).alignment = { horizontal: 'left', vertical: 'middle' }; // Left-aligned to prevent any left-clipping

    sheet.mergeCells(`N${r}:Z${r}`);
    sheet.getCell(`N${r}`).value = semData.track;
    sheet.getCell(`N${r}`).font = { bold: true, size: 10 };
    sheet.getCell(`N${r}`).alignment = { horizontal: 'left', vertical: 'middle' };
    for (let c = 13; c <= 25; c++) {
      sheet.getCell(r, c).border = { bottom: { style: 'thin' } };
    }
    ...
    sheet.getCell(`M${sigR}`).value = semData.principal;
    sheet.getCell(`M${sigR}`).font = { bold: true, size: 10 };
    sheet.getCell(`M${sigR}`).alignment = { horizontal: 'center', vertical: 'bottom' };
    
    // Right signature: Columns W to Y (width 24.3)
    sheet.mergeCells(`X${sigR}:Z${sigR}`);
    sheet.getCell(`X${sigR}`).value = semData.registrar;
    sheet.getCell(`X${sigR}`).font = { bold: true, size: 10 };
    sheet.getCell(`X${sigR}`).alignment = { horizontal: 'center', vertical: 'bottom' };
    ...
    sheet.mergeCells(`M${sigR}:R${sigR}`);
    sheet.getCell(`M${sigR}`).value = 'SHS Head/Principal';
    sheet.getCell(`M${sigR}`).font = { size: 9, bold: false };
    sheet.getCell(`M${sigR}`).alignment = { horizontal: 'center', vertical: 'top' };
    sheet.getCell(`M${sigR}`).border = { top: {style: 'thin'} };
    
    sheet.mergeCells(`X${sigR}:Z${sigR}`);
    sheet.getCell(`X${sigR}`).value = 'Registrar';
    sheet.getCell(`X${sigR}`).font = { size: 9, bold: false };
    sheet.getCell(`X${sigR}`).alignment = { horizontal: 'center', vertical: 'top' };
    sheet.getCell(`X${sigR}`).border = { top: {style: 'thin'} };
  }
